      ...
      ? {
          ...
          ridePerformance:
            parsed.latestReport && typeof parsed.latestReport === "object" && "ridePerformance" in parsed.latestReport
              ? ((parsed.latestReport as Record<string, unknown>).ridePerformance as GameState["monthlyHistory"][number]["ridePerformance"])
              : []
        }
      : null,
      ...
      ? parsed.monthlyHistory.map((report) => ({
          ...
          ridePerformance:
            report && typeof report === "object" && "ridePerformance" in report
              ? ((report as Record<string, unknown>).ridePerformance as GameState["monthlyHistory"][number]["ridePerformance"])
              : []
        }))
      : []
  };
